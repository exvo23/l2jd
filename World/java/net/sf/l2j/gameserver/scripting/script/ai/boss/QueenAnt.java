...
public class QueenAnt extends AttackableAIScript
{
	...
	public QueenAnt()
	{
		...
		switch (GrandBossManager.getInstance().getBossStatus(QUEEN_ANT))
		{
	...
	public String onTimer(String name, Npc npc, Player player)
	{
		...
		if (name.equalsIgnoreCase("queen_unlock"))
			spawnBoss();
	...
	public void onMyDying(Npc npc, Creature killer)
	{
		...
		if (npc.getNpcId() == QUEEN_ANT)
		{
			...
			// Start respawn timer.
			startQuestTimer("queen_unlock", null, null, getRespawnInterval());
	...
	private void spawnBoss()
	{
		...
		queen = (GrandBoss) addSpawn(QUEEN_ANT, info.getInteger("loc_x"), info.getInteger("loc_y"), info.getInteger("loc_z"), info.getInteger("heading"), false, 0, false);
	...
	private static long getRespawnInterval()
	{
		...
		try
		{
			timePattern = new SchedulingPattern(QAConfig.FWA_FIXTIMEPATTERNOFQA);
			long delay = timePattern.next(now) - now;
			return Math.max(60000, delay);
		}
		catch (SchedulingPattern.InvalidPatternException ipe)
		{
			throw new RuntimeException("Invalid respawn data \"" + QAConfig.FWA_FIXTIMEPATTERNOFQA + "\" in " + QueenAnt.class.getSimpleName(), ipe);
		}
	}
}