...
public class Zaken extends AttackableAIScript
{
	...
	{
		if (name.equalsIgnoreCase("zaken_unlock"))
			spawnBoss();
	...
	{
		if (!ZakenConfig.FWA_FIXTIMEPATTERNOFZAKEN.isEmpty())
		{
			if (npc.getNpcId() == ZAKEN)
			{
				...
				// Start respawn timer.
				startQuestTimer("zaken_unlock", null, null, getRespawnInterval());
				_open = false;
				...
				startQuestTimer("CreateOnePrivateEx", npc, null, ((15 + Rnd.get(1)) * 1000));
		}
	...
	private void spawnBoss()
	{
		...
		if (_open)
		{
			...
			}, ZakenConfig.ZAKEN_TIME_DOOR);
			waiter(ZakenConfig.ZAKEN_TIME_DOOR * 60 * 1000);
		}
	...
	private static long getRespawnInterval()
	{
		...
		try
		{
			timePattern = new SchedulingPattern(ZakenConfig.FWA_FIXTIMEPATTERNOFZAKEN);
			long delay = timePattern.next(now) - now;
			return Math.max(60000, delay);
		}
		catch (SchedulingPattern.InvalidPatternException ipe)
		{
			throw new RuntimeException("Invalid respawn data \"" + ZakenConfig.FWA_FIXTIMEPATTERNOFZAKEN + "\" in " + Zaken.class.getSimpleName(), ipe);
		}
	}
		...
		while (startWaiterTime + interval > System.currentTimeMillis() && !_open)
		{
			...
			switch (seconds)
			{
				...
				case 3600:
					if (_open)
					{
						AnnounceZaken(" will be closed in " + (seconds / 60 / 60) + " Hour(s) !");
	...
	{
		World.announceToOnlinePlayers("Zaken Door:" + " " + text, true);
	}
}