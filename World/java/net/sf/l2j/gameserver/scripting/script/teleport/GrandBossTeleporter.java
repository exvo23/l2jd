...
import net.sf.l2j.gameserver.scripting.script.ai.boss.Fafurion;
import net.sf.l2j.gameserver.scripting.script.ai.boss.Lindvior;
...
public class GrandBossTeleporter extends Quest
{
	...
	private static final Location[] FAFURION_OUT =
	{
		new Location(178872, 218714, -14312),
		new Location(178855, 217812, -14312)
	};
	...
	public GrandBossTeleporter()
	{
		...
		addTalkId(13001, 29055, 31859, 31384, 31385, 31540, 31686, 31687, 31759, 31862, 32107, 32109, 128, 129);
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("baium"))
		{
			...
			else if (!player.getInventory().hasItems(BaiumConfig.BAIUM_ITEM_REQUEST))
				htmltext = "31862-03.htm";
			// All is ok, take the item and teleport the player inside.
			else
			{
				takeItems(player, BaiumConfig.BAIUM_ITEM_REQUEST, BaiumConfig.BAIUM_ITEM_REQUEST_CONT);
				...
				player.teleportTo(BaiumConfig.TELEPORT_ENTRANCE[0], BaiumConfig.TELEPORT_ENTRANCE[1] + Rnd.get(-350, 350), BaiumConfig.TELEPORT_ENTRANCE[2], 0);
			}
		}
		else if (event.equalsIgnoreCase("baium_story"))
			htmltext = "31862-02.htm";
		else if (event.equalsIgnoreCase("baium_exit"))
			player.teleportTo(BaiumConfig.TELEPORT_EXIT[0], BaiumConfig.TELEPORT_EXIT[1], BaiumConfig.TELEPORT_EXIT[2], 100);
		
		else if (event.equalsIgnoreCase("31540"))
		{
			if (player.getInventory().hasItems(ValakasConfig.VALAKAS_ITEM_REQUEST))
			{
				takeItems(player, ValakasConfig.VALAKAS_ITEM_REQUEST, ValakasConfig.VALAKAS_ITEM_REQUEST_CONT);
				player.getMemos().set("GrandBossTeleporters_Valakas", true);
				
				if (player.isGM())
				{
					player.teleportTo(189799, -105377, -784, 0);
				}
				else
		...
		else if (event.equalsIgnoreCase("lindvior"))
		{
			if (Fafurion._PlayersInside.size() < FafurionConfig.MAX_PLAYERS_INSIDE_FAFURION)
			{
				if (player.getInventory().hasItems(LindviorConfig.LIDVIOR_ITEM_REQUEST))
				{
					takeItems(player, LindviorConfig.LIDVIOR_ITEM_REQUEST, LindviorConfig.LIDVIOR_ITEM_REQUEST_CONT);
					player.teleportTo(LindviorConfig.LINDVIOR_TELEPORT_IN_LOC_X, LindviorConfig.LINDVIOR_TELEPORT_IN_LOC_Y, LindviorConfig.LINDVIOR_TELEPORT_IN_LOC_Z, 0);
					player.getMemos().set("GrandBossTeleporters_Lindvior", true);
				}
				else
					htmltext = "128-06.htm";
		...
		else if (event.equalsIgnoreCase("lindvior_solo"))
		{
			boolean ok = true;
			if (!player.isNoble())
			{
		...
		else if (event.equalsIgnoreCase("lindvior_party"))
		{
			if (!player.isInParty())
				htmltext = "128-10.htm";
		...
		else if (event.equalsIgnoreCase("fafurion"))
		{
			
			if (player.getInventory().hasItems(FafurionConfig.FAFURION_ITEM_REQUEST))
			{
				takeItems(player, FafurionConfig.FAFURION_ITEM_REQUEST, FafurionConfig.FAFURION_ITEM_REQUEST_CONT);
				player.teleportTo(Rnd.get(FAFURION_OUT), 100);
				player.getMemos().set("GrandBossTeleporters_Fafurion", true);
			}
			else
				htmltext = "129-06.htm";
		}
		
		else if (event.equalsIgnoreCase("fafurion_solo"))
		{
			boolean check = true;
			if (!player.isNoble())
			{
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		{
			case 128:
				status = GrandBossManager.getInstance().getBossStatus(Lindvior.LINDVIOR);
				if (status == Lindvior.FIGHTING)
					htmltext = "128-02.htm";
				else if (status == Lindvior.DEAD)
					htmltext = "128-01.htm";
				else if (status == Lindvior.DORMANT || status == Lindvior.WAITING)
				{
					if (player.getInventory().hasItems(LindviorConfig.LIDVIOR_ITEM_REQUEST))
					{
						
						ZoneManager.getInstance().getZoneById(110018, BossZone.class).allowPlayerEntry(player, 30);
						
						takeItems(player, LindviorConfig.LIDVIOR_ITEM_REQUEST, LindviorConfig.LIDVIOR_ITEM_REQUEST_CONT);
						player.teleportTo(LindviorConfig.LINDVIOR_TELEPORT_IN_LOC_X, LindviorConfig.LINDVIOR_TELEPORT_IN_LOC_Y, LindviorConfig.LINDVIOR_TELEPORT_IN_LOC_Z, 0);
						player.getMemos().set("GrandBossTeleporters_Lindvior", true);
						
						if (status == Lindvior.DORMANT)
						{
							GrandBossManager.getInstance().setBossStatus(Lindvior.LINDVIOR, Lindvior.WAITING);
							ScriptData.getInstance().getQuest("Lindvior").startQuestTimer("beginning", null, null, LindviorConfig.WAIT_TIME_LINDVIOR);
			...
			case 129:
				status = GrandBossManager.getInstance().getBossStatus(Fafurion.FAFURION);
				if (status == Fafurion.FIGHTING)
					htmltext = "129-02.htm";
				else if (status == Fafurion.DEAD)
					htmltext = "129-01.htm";
				else if (status == Fafurion.DORMANT || status == Fafurion.WAITING)
				{
					if (player.getInventory().hasItems(FafurionConfig.FAFURION_ITEM_REQUEST))
					{
						
						ZoneManager.getInstance().getZoneById(110019, BossZone.class).allowPlayerEntry(player, 30);
						
						takeItems(player, FafurionConfig.FAFURION_ITEM_REQUEST, FafurionConfig.FAFURION_ITEM_REQUEST_CONT);
						player.teleportTo(Rnd.get(FAFURION_OUT), 100);
						player.getMemos().set("GrandBossTeleporters_Fafurion", true);
						
						if (status == Fafurion.DORMANT)
						{
							GrandBossManager.getInstance().setBossStatus(Fafurion.FAFURION, Fafurion.WAITING);
							ScriptData.getInstance().getQuest("fafurion").startQuestTimer("beginning", null, null, FafurionConfig.WAIT_TIME_FAFURION);
			...
			case 13001:
				...
				{
					if (player.getInventory().hasItems(AntharasConfig.ANTHARAS_ITEM_REQUEST))
					{
						takeItems(player, AntharasConfig.ANTHARAS_ITEM_REQUEST, AntharasConfig.ANTHARAS_ITEM_REQUEST_CONT);
						ZoneManager.getInstance().getZoneById(110001, BossZone.class).allowPlayerEntry(player, 30);
						
						player.teleportTo(AntharasConfig.TELEPORT_ENTRANCE[0] + Rnd.get(-350, 350), AntharasConfig.TELEPORT_ENTRANCE[1] + Rnd.get(-1000, 1000), AntharasConfig.TELEPORT_ENTRANCE[2], 0);
						
						if (status == Antharas.DORMANT)
						{
							GrandBossManager.getInstance().setBossStatus(Antharas.ANTHARAS, Antharas.WAITING);
							ScriptData.getInstance().getQuest("Antharas").startQuestTimer("beginning", null, null, AntharasConfig.WAIT_TIME_ANTHARAS);
						}
					}
					else
				...
				if (status == Valakas.DORMANT || status == Valakas.WAITING)
				{
					...
					else if (player.getMemos().containsKey("GrandBossTeleporters_Valakas"))
					{
						...
						if (status == Valakas.DORMANT)
						{
							...
							ScriptData.getInstance().getQuest("Valakas").startQuestTimer("beginning", null, null, ValakasConfig.WAIT_TIME_VALAKAS);
						}
					}
					else
