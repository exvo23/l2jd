...
public class Frintezza extends AttackableAIScript
{
	...
	private static final int FRINTEZZA_SCROLL = FrintezzaConfig.FRINTEZZA_ITEM_REQUEST; // Frintezza's Magic Force Field Removal Scroll.
	...
	public String onTimer(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("camera_1"))
		{
			...
			_frintezzaDummy = addSpawn(DUMMY, 174240, -89805, -5022, 16048, false, 0, false);
			_frintezzaDummy.setInvul(true);
			_frintezzaDummy.setIsImmobilized(true);
			_overheadDummy = addSpawn(DUMMY, 174232, -88020, -5110, 16384, false, 0, false);
			...
			_portraitDummy1 = addSpawn(DUMMY, 172450, -87890, -5100, 16048, false, 0, false);
			_portraitDummy1.setIsImmobilized(true);
			_portraitDummy1.setInvul(true);
			_portraitDummy3 = addSpawn(DUMMY, 176012, -87890, -5100, 16048, false, 0, false);
			...
			startQuestTimer("stop_pc", npc, null, 0);
			startQuestTimer("camera_2", _overheadDummy, null, 1000);
		}
		...
		else if (event.equalsIgnoreCase("camera_3"))
		{
			...
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 0, 75, -89, 0, 100, 0, 0, 1, 0));
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 0, 75, -89, 0, 100, 0, 0, 1, 0));
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 300, 90, -10, 6500, 7000, 0, 0, 1, 0));
			
			_frintezza.setIsImmobilized(true);
			_frintezza.setInvul(true);
			_frintezza.disableAllSkills();
			_demon2 = (Monster) addSpawn(29051, 175876, -88713, -5100, 28205, false, 0, false);
			_demon2.setIsImmobilized(true);
			_demon2.disableAllSkills();
			_demon3 = (Monster) addSpawn(29051, 172608, -88702, -5100, 64817, false, 0, false);
			_demon3.setIsImmobilized(true);
			_demon3.disableAllSkills();
			_demon1 = (Monster) addSpawn(29050, 175833, -87165, -5100, 35048, false, 0, false);
			_demon1.setIsImmobilized(true);
			_demon1.disableAllSkills();
			_demon4 = (Monster) addSpawn(29050, 172634, -87165, -5100, 57730, false, 0, false);
			_demon4.setIsImmobilized(true);
			_demon4.disableAllSkills();
			startQuestTimer("camera_4", _overheadDummy, null, 6500);
		...
		else if (event.equalsIgnoreCase("camera_9b"))
		{
			for (Npc mob : FRINTEZZA_LAIR.getKnownTypeInside(Npc.class))
			{
				if (mob.getNpcId() == 29051 || mob.getNpcId() == 29050)
					FRINTEZZA_LAIR.broadcastPacket(new SocialAction(mob, 1));
			}

			startQuestTimer("camera_10", _frintezza, null, 2000);
		}
		...
		else if (event.equalsIgnoreCase("camera_10"))
		{
			...
			startQuestTimer("camera_11", _frintezza, null, 0);
		}
		else if (event.equalsIgnoreCase("camera_11"))
		{
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(_frintezza.getObjectId(), 240, 90, 25, 5500, 10000, 0, 0, 1, 0));
			...
			startQuestTimer("camera_11b", npc, null, 4100);
			startQuestTimer("camera_12", _frintezza, null, 4500L);
		}
		...
		else if (event.equalsIgnoreCase("camera_18b"))
		{
			...
			_weakScarlet = (GrandBoss) addSpawn(SCARLET1, 174232, -88020, -5110, 20458, false, 0L, true);
			...
			Player victim = getRandomPlayer(_scarletDummy);
			if (victim != null)
				_scarletDummy.forceAttack(victim, 200);
			
		}
		else if (event.equalsIgnoreCase("camera_19"))
			_weakScarlet.teleportTo(174232, -88020, -5110, 0);
		...
		else if (event.equalsIgnoreCase("camera_20"))
		{
			...
			startQuestTimer("camera_21", _weakScarlet, null, 2000);
		}
		else if (event.equalsIgnoreCase("camera_21"))
		{
			...
			startQuestTimer("camera_22", _weakScarlet, null, 3000);
		}
		else if (event.equalsIgnoreCase("camera_22"))
		{
			...
			startQuestTimer("loc_check", _weakScarlet, null, 60000);
		...
		else if (event.equalsIgnoreCase("camera_23"))
		{
			...
			FRINTEZZA_LAIR.broadcastPacket(new MagicSkillUse(_frintezza, _frintezza, 5006, 1, 34000, 0));
			startQuestTimer("songs_play", _frintezza, null, 10000 + Rnd.get(10000));
			startQuestTimer("skill01", _weakScarlet, null, 10000 + Rnd.get(10000));
		}
		...
		else if (event.equalsIgnoreCase("start_pc"))
		{
			for (Creature cha : FRINTEZZA_LAIR.getKnownTypeInside(Player.class))
			{
				...
				cha.setIsImmobilized(false);
				cha.setInvul(false);
				cha.sendPacket(new CameraMode(0));
				cha.sendPacket(NormalCamera.STATIC_PACKET);
			}
		}
		...
		if (event.equalsIgnoreCase("morph_01"))
		{
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(_weakScarlet.getObjectId(), 250, _angle, 12, 2000, 11000, 0, 0, 1, 0));
			startQuestTimer("morph_02", _weakScarlet, null, 3000);
		}
		else if (event.equalsIgnoreCase("morph_02"))
		{
			...
			startQuestTimer("morph_03", _weakScarlet, null, 1500);
		}
		else if (event.equalsIgnoreCase("morph_03"))
		{
			startQuestTimer("morph_04", _weakScarlet, null, 1500);
		}
		else if (event.equalsIgnoreCase("morph_04"))
		{
			...
			startQuestTimer("morph_end", _weakScarlet, null, 1);
			startQuestTimer("start_pc", _weakScarlet, player, 1000);
			startQuestTimer("start_npc", _weakScarlet, player, 1000);
			startQuestTimer("songs_play", _frintezza, null, 10000 + Rnd.get(10000));
			startQuestTimer("skill02", _frintezza, null, 10000 + Rnd.get(10000));
		...
		else if (event.equalsIgnoreCase("morph_06"))
		{
			...
			Player victim = getRandomPlayer(_weakScarlet);
			if (victim != null)
				_weakScarlet.forceAttack(_weakScarlet, 200);
		...
		else if (event.equalsIgnoreCase("morph_13"))
		{
			...
			startQuestTimer("loc_check", _strongScarlet, null, 60000);
			
			Player victim = getRandomPlayer(_strongScarlet);
			if (victim != null)
				_strongScarlet.forceAttack(victim, 200);
		}
		...
		else if (event.equalsIgnoreCase("morph_15"))
		{
			...
			startQuestTimer("morph_end", _weakScarlet, null, 9000);
			
			startQuestTimer("start_pc", _weakScarlet, player, 6000);
			startQuestTimer("start_npc", _weakScarlet, player, 6000);
		...
		else if (event.equalsIgnoreCase("morph_16"))
		{
			...
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(_strongScarlet.getObjectId(), 200, _angle, 85, 4000, 10000, 0, 0, 1, 0));
			startQuestTimer("morph_17b", _frintezza, null, 7400);
			startQuestTimer("morph_18", _strongScarlet, null, 7400);
		}
		
		else if (event.equalsIgnoreCase("morph_18"))
		{
			
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(_strongScarlet.getObjectId(), 100, 120, 5, 7500, 10000, 0, 0, 1, 0));
			startQuestTimer("morph_19", _frintezza, null, 0);
		}
		
		else if (event.equalsIgnoreCase("morph_19"))
		{
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(_frintezza.getObjectId(), 100, 90, 5, 5000, 15000, 0, 0, 1, 0));
		...
		{
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(_frintezza.getObjectId(), 100, 90, 5, 1, 1, 0, 0, 1, 0));
			startQuestTimer("start_pc", npc, null, 7000L);
			FRINTEZZA_LAIR.broadcastPacket(new SpecialCamera(_frintezza.getObjectId(), 900, 90, 25, 1, 1, 0, 0, 1, 0));
		...
		else if (event.equalsIgnoreCase("songs_play"))
		{
			...
			if (_frintezza != null && !_frintezza.isDead() && _onMorph == 0)
			{
				...
				FRINTEZZA_LAIR.broadcastPacket(new ExShowScreenMessage(SongName, 6000, SMPOS.BOTTOM_RIGHT, false));
				...
				if (_onSong == 1 && _thirdMorph == 1 && _strongScarlet.getStatus().getHp() < _strongScarlet.getStatus().getMaxHp() * 0.6D && Rnd.get(100) < 80)
				{
					...
					startQuestTimer("songs_play", _frintezza, null, 32000 + Rnd.get(10000));
				}
				...
				else if (_onSong == 4 && _secondMorph == 1)
				{
					...
					startQuestTimer("songs_effect", _frintezza, null, 5000L);
					startQuestTimer("songs_play", _frintezza, null, (31000 + Rnd.get(10000)));
				}
				else if (_onSong == 5 && _thirdMorph == 1 && _abnormal == 0)
				{
					...
					startQuestTimer("songs_effect", _frintezza, null, 5000L);
					startQuestTimer("songs_play", _frintezza, null, 35000 + Rnd.get(10000));
				}
				else
					startQuestTimer("songs_play", _frintezza, null, 5000 + Rnd.get(50000));
			}
		}
			...
			else if (_onSong == 5)
			{
				for (Creature cha : FRINTEZZA_LAIR.getKnownTypeInside(Player.class))
				{
					if (Rnd.get(100) < 70)
					{
						...
						cha.getAI().tryToIdle();
						skill.getEffects(_frintezza, cha);
						FRINTEZZA_LAIR.broadcastPacket(new ExShowScreenMessage(skill.getName(), 6000));
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		else if (GrandBossManager.getInstance().getBossStatus(FRINTEZZA) == DORMANT)
		{
			...
			if (party_check_success)
			{
				...
				{
					player.destroyItemByItemId("Quest", FRINTEZZA_SCROLL, FrintezzaConfig.FRINTEZZA_ITEM_REQUEST_CONT, player, true);
	...
	public void onAttacked(Npc npc, Creature attacker, int damage, L2Skill skill)
	{
		...
		if (npc.getNpcId() == SCARLET1 && _secondMorph == 0 && _thirdMorph == 0 && _onMorph == 0 && npc.getStatus().getHp() < npc.getStatus().getMaxHp() * 0.75D && GrandBossManager.getInstance().getBossStatus(FRINTEZZA) == FIGHTING)
		{
			...
			_secondMorph = 1;
			_onMorph = 1;
			startQuestTimer("attack_stop", npc, null, 0L);
			startQuestTimer("stop_pc", npc, null, 1000L);
			startQuestTimer("stop_npc", npc, null, 1000L);
			// startQuestTimer("morph_01", npc, null, 1100L);
			
			startQuestTimer("morph_01", npc, null, 1000);
			startQuestTimer("morph_02", npc, null, 3400);
			startQuestTimer("morph_03", npc, null, 5800);
			startQuestTimer("morph_04", npc, null, 8200);
		...
		else if (npc.getNpcId() == SCARLET1 && _secondMorph == 1 && _thirdMorph == 0 && _onMorph == 0 && npc.getStatus().getHp() < npc.getStatus().getMaxHp() * 0.5D && GrandBossManager.getInstance().getBossStatus(FRINTEZZA) == FIGHTING)
		{
			...
			_thirdMorph = 1;
			_onMorph = 1;
			startQuestTimer("stop_pc", npc, null, 2000);
			startQuestTimer("stop_npc", npc, null, 2000);
	...
	public void onMyDying(Npc npc, Creature killer)
	{
		...
		if (npc.getNpcId() == SCARLET2)
		{
			...
			startQuestTimer("check_hp", null, null, 0L);
			FRINTEZZA_LAIR.broadcastPacket(new PlaySound("BS01_D"));
			startQuestTimer("attack_stop", _frintezza, null, 0L);
			startQuestTimer("stop_pc", npc, null, 0L);
			startQuestTimer("stop_npc", npc, null, 0L);
		...
		else if (npc.getNpcId() == 18334)
		{
			...
			if (_killDarkChoirCaptain == 8)
			{
				...
				startQuestTimer("waiting", null, null, FrintezzaConfig.WAIT_TIME_FRINTEZZA);
				cancelQuestTimers("room_final");
			}
		}
	...
	private static long getRespawnInterval()
	{
		...
		try
		{
			timePattern = new SchedulingPattern(FrintezzaConfig.FWA_FIXTIMEPATTERNOFFRINTEZZA);
			long delay = timePattern.next(now) - now;
			return Math.max(60000, delay);
		}
		catch (SchedulingPattern.InvalidPatternException ipe)
		{
			throw new RuntimeException("Invalid respawn data \"" + FrintezzaConfig.FWA_FIXTIMEPATTERNOFFRINTEZZA + "\" in " + Frintezza.class.getSimpleName(), ipe);
		}
	}
