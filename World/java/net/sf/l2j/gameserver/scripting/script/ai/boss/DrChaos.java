...
public class DrChaos extends AttackableAIScript
{
	...
	public void onMyDying(Npc npc, Creature killer)
	{
		...
		startQuestTimer("reset_drchaos", null, null, getRespawnInterval());
	...
	private static long getRespawnInterval()
	{
		...
		try
		{
			timePattern = new SchedulingPattern(DrChaosConfig.FWA_FIXTIMEPATTERNOFCHAOS);
			long delay = timePattern.next(now) - now;
			return Math.max(60000, delay);
		}
		catch (SchedulingPattern.InvalidPatternException ipe)
		{
			throw new RuntimeException("Invalid respawn data \"" + DrChaosConfig.FWA_FIXTIMEPATTERNOFCHAOS + "\" in " + DrChaos.class.getSimpleName(), ipe);
		}
	}
}