...
public class EventConfig
{
	...
	public FastMap<Integer, FastMap<String, FastList<Integer>>> door;
	...
	private void addDoor(int id, String type, String ids)
	{
		if (!door.containsKey(id))
			door.put(id, new FastMap<String, FastList<Integer>>());
	...
	public FastList<Integer> getDoor(int event, String type)
	{
		if (!(door.containsKey(event)))
		{
			_log.warning("Event: Try to get a restriction of a non existing event: ID: " + event);
			return null;
		}
		if (door.get(event).containsKey(type))
			return door.get(event).get(type);
	...
	{
		File configFile = new File("./data/xml/events/EventsManager.xml");
		Document doc = null;
		try
		{
					...
					for (Node event = root.getFirstChild(); event != null; event = event.getNextSibling())
					{
						if ("event".equalsIgnoreCase(event.getNodeName()))
						{
							...
							for (Node property = event.getFirstChild(); property != null; property = property.getNextSibling())
							{
								...
								else if ("attakdoor".equalsIgnoreCase(property.getNodeName()))
								{
									NamedNodeMap propAttrs = property.getAttributes();
									String type = propAttrs.getNamedItem("type").getNodeValue();
									String ids = propAttrs.getNamedItem("ids").getNodeValue();
									addDoor(eventId, type, ids);
								}
