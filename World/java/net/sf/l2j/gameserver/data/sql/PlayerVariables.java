...
public class PlayerVariables
{
	...
	public static void changeValue(Player player, String name, String value)
	{
		...
		Maria.set("UPDATE character_memo_alt SET value=? WHERE obj_id=? AND name=?", value, player.getObjectId(), name);
	}
	...
	public static void setVar(Player player, String name, String value, long expirationTime)
	{
		...
		player.getVariables().put(name, new PlayerQuestVar(player, name, value, expirationTime));
		Maria.set("REPLACE INTO character_memo_alt (obj_id, name, value, expire_time) VALUES (?,?,?,?)", player.getObjectId(), name, value, expirationTime);
	}
	
	public static void setVar(int objId, String name, String value, long expirationTime)
	{
		Maria.set("REPLACE INTO character_memo_alt (obj_id, name, value, expire_time) VALUES (?,?,?,?)", objId, name, value, expirationTime);
	}
	...
	public static PlayerQuestVar getVarObject(Player player, String name)
	{
	...
	public static void unsetVar(Player player, String name)
	{
		...
		PlayerQuestVar pv = player.getVariables().remove(name);
		...
		if (pv != null)
		{
			...
			else if (name.equals("dungeon_atleast1time"))
			{
				pv.getOwner().broadcastCharInfo();
				pv.getOwner().broadcastUserInfo();
			}
			Maria.set("DELETE FROM character_memo_alt WHERE obj_id=? AND name=? LIMIT 1", pv.getOwner().getObjectId(), name);
	...
	public static void deleteExpiredVar(Player player, String name, String value)
	{
		...
		if (name.contains("skin"))
			player.setDeleteTimer(Integer.parseInt(value));
		
		Maria.set("DELETE FROM character_memo_alt WHERE obj_id=? AND name=? LIMIT 1", player.getObjectId(), name);
	}
	
	public static String getVar(Player player, String name)
	{
		PlayerQuestVar pv = getVarObject(player, name);
	...
	public static void votedResult(Player player)
	{
		...
		{
			Maria.closeQuietly(con, offline, rs);
		}
	}
	...
	public static void loadVariables(Player player)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				player.getVariables().put(name, new PlayerQuestVar(player, name, value, expire_time));
			}
