...
public final class RequestSellItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		final Npc merchant = ((Npc) player.getTarget());
		...
		for (IntIntHolder i : _items)
		{
			...
			{
				price = Config.ALL_ITEMS_SELL_0_ADENA;
			}
			else
				price = item.getReferencePrice() / 2;
			
			if (Config.ALL_ITEM_0_ADENA)
				totalPrice += price * Config.ALL_ITEMS_SELL_0_ADENA;
			else
				totalPrice += price * i.getValue();
			
			if ((Integer.MAX_VALUE / i.getValue()) < price || totalPrice > Integer.MAX_VALUE)
			{
				LOGGER.info("BAN-L2JDev: " + player.getName() + " of account " + player.getAccountName() + " tried to purchase over " + Integer.MAX_VALUE + " adena worth of goods.");
				return;
			}
			player.getInventory().destroyItem("Sell", i.getId(), i.getValue(), player, merchant);
		}
		if (Config.ALL_ITEM_0_ADENA)
			player.addAdena("Sell", Config.ALL_ITEMS_SELL_0_ADENA, merchant, false);
		else
