...
public final class RequestCharacterCreate extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (Config.ENABLE_SPAWN_START_PLAYER)
			player.getPosition().set(Config.SPAWN_START_TUTORIAL_LOCATION[0], Config.SPAWN_START_TUTORIAL_LOCATION[1], Config.SPAWN_START_TUTORIAL_LOCATION[2]);
		else
			player.getPosition().set(template.getRandomSpawn());
		
		player.setTitle(Config.NEW_PLAYER_TITLE);
		
		int newLevel = Config.PLAYER_START_CREATION_LEVEL;
		PlayerLevel pl = PlayerLevelData.getInstance().getPlayerLevel(newLevel);
		...
		player.getMemos().set("vipEndTime", 0);
		player.getMemos().set("heroEndTime", 0);
		player.getMemos().set("startEndTime", false);
		player.getMemos().set("startEndTimeArmor", false);
