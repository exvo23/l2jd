...
public class MultiSellChoose extends L2GameClientPacket
{
	...
	public void runImpl()
	{
		...
		for (Ingredient e : ingredientsList)
		{
			...
			if (e.getItemId() != PC_BANG_POINTS && e.getItemId() != PC_BANG_POINTS)
			{
			...
			else
			{
				...
				if (e.getItemId() == PC_BANG_POINTS && e.getItemCount() * _amount > player.getPcBangScore())
				{
					
					player.sendMessage("You don't have enough PC Bang Points.");
					return;
		...
		for (Ingredient e : entry.getIngredients())
		{
			...
			if (e.getItemId() != CLAN_REPUTATION && e.getItemId() != PC_BANG_POINTS)
			{
			...
			else
			{
				...
				else
				{
					...
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.USING_S1_PCPOINT).addNumber(e.getItemCount()));
				}
			}
		...
		for (Ingredient e : entry.getProducts())
		{
			...
			if (ItemData.getInstance().createDummyItem(e.getItemId()).isStackable())
