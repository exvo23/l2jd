		...
		try
		{
			ThreadPool.execute(() ->
			{
				...
				if (getPlayer() != null && !isDetached())
				{
					...
					EventManager.getInstance().unregisterPlayer(getPlayer());
					if (OfflineTable.getInstance().canBeOffline(player))
					{
						if (getPlayer().getParty() != null)
							getPlayer().getParty().removePartyMember(player, MessageType.EXPELLED);
						
						OlympiadManager.getInstance().unRegisterNoble(player);
						
						final Summon summon = getPlayer().getSummon();
						if (summon!= null)
						{
							summon.doRevive();
						...
						OfflineTable.getInstance().applyEffect(getPlayer());
