...
public class Olympiad
{
	...
	private final static Map<Integer, StatSet> _nobles = new ConcurrentHashMap<>();
	...
	private static final String SELECT_CLASSIFIED_NOBLES = "SELECT char_id from olympiad_nobles WHERE competitions_done >= ? ORDER BY olympiad_points DESC, competitions_done DESC, competitions_won DESC";
	...
	public static StatSet getNobleStats(int objectId)
	{
	 ...
	 */
	private static void saveNobleData()
	{
		if (_nobles.isEmpty())
			return;
	...
	protected void setNewOlympiadEndCustom()
	{
		...
		switch (Config.ALT_OLY_PERIOD)
		{
			case "DAY":
			{
				...
				{
					LOGGER.info("Invalid config value for Config.ALT_OLY_PERIOD_MULTIPLIER, must be >= 7");
				}
				break;
			}
			case "WEEK":
	...
	public static void olympiadEnd(Player player)
	{
		...
		if (player.getStatus().getLevel() >= 76 && player.isNoble())
		player.sendPacket(new CreatureSay(0, SayType.CRITICAL_ANNOUNCE, player.getName(), "" + "Olympiad period ends in " + numDays + " days, " + numHours + " hours and " + numMins + " mins."));
	}
