...
public abstract class Item
{
	...
	protected Item(StatSet set)
	{
		...
		_heroItem = (_itemId >= 0 && _itemId <= 0) || _itemId == 0;
	...
	public boolean isOlyRestrictedItemList()
	{
		
		return (Config.LIST_OLY_RESTRICTED_ITEMS.contains(getItemId()));
	}
	public boolean isOlyRestrictedItemByGrade()
	{
		
		return ((Config.ALT_OLY_GRADE_A) && (getCrystalType() == CrystalType.S));
	}
	
	public boolean checkCondition(Creature creature, WorldObject object, boolean sendMessage)
	{
		// Don't allow hero equipment and restricted items during Olympiad
		if ((isOlyRestrictedItem() || isOlyRestrictedItemList() || isOlyRestrictedItemByGrade() || isHeroItem()) && (creature instanceof Player && creature.getActingPlayer().isInOlympiadMode()))
		{
			if (isEquipable())
	...
	{
		return _crystalType.equals(CrystalType.NONE) ? "-" : _crystalType.toString();
	}
