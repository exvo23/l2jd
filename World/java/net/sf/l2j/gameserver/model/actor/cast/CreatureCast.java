			...
			for (final Creature target : targets)
			{
				...
				switch (skill.getTargetType())
				{
					...
					case HOLY_FOS:
						...
						if (skill.getTargetType() == SkillTargetType.HOLY_FOS || EventManager.checkIfOkToCastSealOfRule(player))
						{
							if (EventManager.getInstance().isRegistered(player) && EventManager.getInstance().isFortressManager())
							{
								EventManager.getInstance().getCurrentEvent().FortressReset(player);
							}
