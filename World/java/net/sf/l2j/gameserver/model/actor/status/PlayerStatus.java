...
public class PlayerStatus extends PlayableStatus<Player>
{
	...
	public final boolean addLevel(byte value)
	{
		...
		if (levelIncreased)
		{
			// if (Config.DISABLE_ALL_TUTORIAL_RETAIL)
			{
				// final QuestState qs = _actor.getQuestList().getQuestState("Tutorial");
				// if (qs != null)
			...
			if (_actor.getStatus().getLevel() >= 20)
			{
				ClassMaster.showTutorialHtml(_actor);
			}

			if (_actor.getStatus().getLevel() == 22)
				SpecialTutorialTeleport.showQuestionTeleport(_actor);
			
			if (_actor.getStatus().getLevel() == 25)
				SpecialTutorialTeleport.showQuestionTeleport(_actor);
