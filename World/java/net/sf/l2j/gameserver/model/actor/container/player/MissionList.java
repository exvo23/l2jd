...
public class MissionList
{
	...
	public void restore()
	{
		...
		{
			LOGGER.warn("Couldn't load mission list for player {}:", e, _player.getName());
		}
	}
	...
	public void store()
	{
		...
		{
			LOGGER.warn("Couldn't store missions infos for player {}:", e, _player.getName());
		}
	}
	...
	public void set(MissionType type, int value, boolean increaseValue, boolean resetValue)
	{
		...
		if (missionData.getRequired() <= mission.getValue())
		{
			if (missionData.getRewards() != null && !missionData.getRewards().isEmpty())
				missionData.getRewards().forEach(reward -> _player.addItem("MissionReward", reward.getId(), reward.getValue(), _player, true));
			...
			_player.sendPacket(new CreatureSay(SayType.TELL, "Mission System", "Lv " + missionData.getLevel() + " " + missionData.getName() + " mission completed."));
		}
