...
public class PlayerCast extends PlayableCast<Player>
{
	...
	public boolean canDoCast(Creature target, L2Skill skill, boolean isCtrlPressed, int itemObjectId)
	{
		...
		if (!isArea)
		{
			if (EventManager.getInstance().isRegistered(_actor))
			{
				if (!EventManager.getInstance().getCurrentEvent().canAttack(_actor, target))
				{
				...
				if (target instanceof Player && EventManager.getInstance().areTeammates(_actor, (Player) target) && skill.isOffensive())
				{
					if (_actor.getAI().getCurrentIntention().getType() != IntentionType.ACTIVE && _actor.getAI().getCurrentIntention().getType() != IntentionType.FOLLOW)
						return false;
		...
		switch (skill.getSkillType())
		{
			...
			case TAKE_FOS:
				if (skill.getTargetType() != SkillTargetType.HOLY_FOS && target instanceof HolyThing)
				{
					if (EventManager.getInstance().isRegistered(_actor) && EventManager.getInstance().isFortressManager())
					{
						int buffid = EventManager.getInstance().getCurrentEvent().getInt("SealFortressSkill");
						
						_actor.broadcastPacketInRadius(new MagicSkillUse(_actor, _actor, buffid, SkillTable.getInstance().getMaxLevel(buffid), skill.getHitTime(), skill.getReuseDelay()), 3000);
						
						_actor.getAI().tryToCast((Creature) _actor.getTarget(), skill);
