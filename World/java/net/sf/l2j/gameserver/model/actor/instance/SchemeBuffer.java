...
public class SchemeBuffer extends Folk
{
	...
	private static final String _fighterSet = Config.BUFFER_FIGHTER_SET;
	private static final String _mageSet = Config.BUFFER_MAGE_SET;
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (currentCommand.equalsIgnoreCase("cubicId"))
		{
			...
			cubicId = Integer.valueOf(st.nextToken());
			bufflevel = Integer.valueOf(st.nextToken());
			player.getCubicList().addOrRefreshCubic(cubicId, bufflevel, 1800, 1000, 50, 1000, false);
		...
		else if (currentCommand.equalsIgnoreCase("getbuff"))
		{
			...
			if (buffid != 0)
			{
				...
				player.stopSkillEffects(buffid);
		...
		else if (currentCommand.startsWith("set"))
		{
			...
			int ids = Integer.parseInt(command.substring(4));
			String[] set_splitted = ids == 1 ? _fighterSet.split(";") : _mageSet.split(";");
		...
		else if (currentCommand.equalsIgnoreCase("buff"))
		{
			int buffid = 0;
			...
			if (buffid != 0)
			{
				...
				if (skill != null)
				{
					...
					MagicSkillUse mgc = new MagicSkillUse(this, player, buffid, SkillTable.getInstance().getMaxLevel(buffid), 1150, 0);
		...
		else if (currentCommand.startsWith("cleanup"))
		{
			...
			html.setFile(getHtmlPath(getNpcId(), 0));
			html.replace("%objectId%", getObjectId());
			html.replace("%buffing%", player.getBuff() == 0 ? "Yourself" : "Your pet");
			html.replace("%count%", player.getBuffCount() + " / " + player.getMaxBuffCount());
			html.replace("%ServerName%", "" + Config.SERVER_NAME);
			player.sendPacket(html);
		}
		else if (currentCommand.startsWith("changebuff"))
		{
	 ...
	 */
	public void showGiveBuffsWindow(Player player)
	{
