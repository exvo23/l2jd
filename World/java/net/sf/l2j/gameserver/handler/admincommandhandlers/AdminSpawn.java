...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	public void useAdminCommand(String command, Player player)
	{
		...
		else if (command.startsWith("admin_unspawnall"))
		{
			...
			SpawnTable.getInstance().despawn();
		...
		else if (command.startsWith("admin_delete"))
		{
			...
			if (!(targetNpc.getSpawn() instanceof Spawn))
				return;
			
			final Spawn spawn = (Spawn) targetNpc.getSpawn();
			spawn.doDelete();
	...
	private void spawn(Player player, String monsterId, int respawnTime, boolean permanent)
	{
		...
		if (template.getNpcId() == 59028 && template.getNpcId() == 25512 && template.getNpcId() == 29001 && template.getNpcId() == 29006 && template.getNpcId() == 29014 && template.getNpcId() == 29019 && template.getNpcId() == 29020 && template.getNpcId() == 29022 && template.getNpcId() == 29028 && template.getNpcId() == 29045 && template.getNpcId() == 29046 && template.getNpcId() == 29047 && template.getNpcId() == 29065)
		{
			player.sendMessage("Not Spawned " + template.getName() + ".");
			return;
		}
		try
		{
			...
			if (RaidBossSpawnManager.getInstance().getValidTemplate(spawn.getNpcId()) != null)
			{
				...
				spawn.doSpawn(false);
				spawn.setRespawnDelay(43200);
				spawn.setRespawnRandom(129600);
